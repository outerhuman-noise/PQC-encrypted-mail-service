...
def enabled_algorithms():
    ...
    return {
        ...
        "selected_kem": Config.KEM_ALG,
        "selected_signature": Config.SIG_ALG,
    }
...
def decapsulate(receiver_private_key_b64: str, kem_ciphertext_b64: str):
    require_oqs()
    with oqs.KeyEncapsulation(Config.KEM_ALG, secret_key=unb64(receiver_private_key_b64)) as kem:
        return kem.decap_secret(unb64(kem_ciphertext_b64))


def generate_sign_keypair():
    require_oqs()
    with oqs.Signature(Config.SIG_ALG) as sig:
...
def sign_message(private_key_b64: str, message: bytes) -> str:
    require_oqs()
    with oqs.Signature(Config.SIG_ALG, secret_key=unb64(private_key_b64)) as sig:
